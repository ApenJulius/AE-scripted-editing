...
var s = {
    "names":["isak", "Jonas", "Julian", "Joseph", "Vetle", "Kasper"]
}
...
function renderQ(comp) {
    var item = app.project.renderQueue.items.add(comp)
    var outputModule = item.outputModule(1)

    var outputFolder = "~/Desktop"
    outputModule.applyTemplate("MOV")
    outputModule.file = File(outputFolder + "/TEXT" + comp.name)
...
var text = window.addEventListener("statictext", undefined, "Run this to get all your videos")

var group = window.add("group", undefined, "")
group.orientation = "row";
var button = group.add("button", undefined, "button")
...
function editText() {
    ...
    var myFile = new File ("C:\\Code\\VideoEditingCode")
        if(myFile.open("r")) {
            myFile.encoding = "UTF-8"
            var myJson = myFile.read()
            var myObject = JSON.parse(myJson)
            alert("open")
            alert(myObject.names)
            myFile.close()Adobe After Effects 8.0 Keyframe Data
...
Data	Outline #1	matchName0	matchName1
	Frame		
	

Expression Data
comp("Comp 1").layer("data.json").data("Outline").matchName0.matchName1
...
End of Keyframe Data
    ...
    s.forEach(e =>
        e.foreach(e) => {
             layer.property("Source Text").setValue(e)
            renderQ(comp)
        }
       
    )
    //sente = `Have a wonderful day, ${}`
